...
const App: React.FC = () => {
	...
	const updateGameState = async (updater: (prevState: GameState) => GameState) => {
		...
		try {
			...
			const progressEntries = Object.entries(newState.progress).map(([key, completed]) => ({
				user_id: user.id,
				progress_key: key,
				completed
			}));
	...
	const handleSignOut = async () => {
		...
		setGameState({ progress: {}, totalScore: 0 });
	};
	...
	if (!user) {
		return <Auth onAuthSuccess={loadUserProgress} />;
	}
				...
				<div>
					<div className="flex justify-between items-center mb-6">
						...
						<div className="flex gap-2">
							...
							<button onClick={handleSignOut} className="bg-red-700 hover:bg-red-600 text-xs font-bold py-1 px-2 rounded">
								Salir
							</button>
						</div>
					</div>
